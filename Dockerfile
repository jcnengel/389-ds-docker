...
RUN apk add build-base wget nspr-dev nss-dev openldap-dev db-dev \
    ...
    python3 py3-setuptools py3-ldap3 py3-asn1 py3-dateutil py3-pip
    py3-argcomplete
...
RUN apk del build-base nspr-dev nss-dev openldap-dev db-dev \
    ...
    cmocka-dev libevent-dev krb5-dev py3-pip

ENV ROOT_PW ''
ENV INSTANCE_NAME localhost
ENV BASEDN dc=example,dc=org

RUN /usr/bin/dscreate create-template /etc/dirsrv/ds.inf && \
    sed -i \
